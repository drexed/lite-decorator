...
RSpec.describe Lite::Decorator::Base do
  ...
  describe '.object' do
    it 'to be a Fake instance object' do
      expect(fake_decorator.object.is_a?(Fake)).to eq(true)
    end

    it 'to be a User instance object' do
      expect(user_decorator.object.is_a?(User)).to eq(true)
    end
  end

  describe '.to_model' do
    it 'to be a Fake object' do
      expect(fake_decorator.to_model.is_a?(Fake)).to eq(true)
    end

    it 'to be a User object' do
      expect(user_decorator.to_model.is_a?(User)).to eq(true)
    end
  end
